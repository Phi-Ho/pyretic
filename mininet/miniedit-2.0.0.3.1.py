...
Version 2.0.0.1
Updates by Gregory Gee
- Added Save and Load of topologies
- Choosing Reference or Remote controller
Version 2.0.0.2
Updates by Gregory Gee
- Added Link properties.  Double click on link and set TCLink parameters for 'bw' and 'delay'
Version 2.0.0.3
Updates by Gregory Gee
- Hosts and switches are now numbered separately
- From Edit->Preferences, you can set the IP Base
- You can export your topology to a mininet custom topology py file.
...
from mininet.util import ipStr, netParse, ipAdd
...
class PrefsDialog(tkSimpleDialog.Dialog):
        ...
        def body(self, master):
            ...
            Label(master, text="IP Base:").grid(row=0)
...
class LinkDialog(tkSimpleDialog.Dialog):
        ...
        def body(self, master):
            ...
            self.var = StringVar(master)
            Label(master, text="Bandwidth:").grid(row=0)
            Label(master, text="Delay:").grid(row=1)
...
class MiniEdit( Frame ):
    ...
    def saveTopology( self ):
        ...
        if len(fileName ) > 0:
            ...
            for link in self.links.values():
                ...
                fout.writerow(["l",srcName,dstName, bw, delay])
    ...
    def exportTopology( self ):
        ...
        if len(fileName ) > 0:
            ...
            f.write("#   sudo mn --custom "+fileName+" --topo mytopo")
            if optsExist:
                f.write(" --link tc")
            f.write("\n")
            f.write("\n")
    ...
    def createLinkBindings( self ):
        ...
        def linkDetails( _event, link=self.link ):
            "Link Details."
        ...
        self.canvas.tag_bind( self.link, '<Double-ButtonPress-1>', linkDetails )
    ...
    def prefDetails( self ):
        prefDefaults = {'ipBase':self.minieditIpBase}
